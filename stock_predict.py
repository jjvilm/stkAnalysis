...
def patternStorage():
    ...
    print('Pattern Storage took: {} seconds'.format(patEndTime-patStartTime))
...
def patternRecognition():
    ...
    for eachPattern in patternAr:
        ...
        sim11 = 100.00 -  abs(percentChange(eachPattern[10], patForRec[0]))
        sim12 = 100.00 -  abs(percentChange(eachPattern[11], patForRec[1]))
        sim13 = 100.00 -  abs(percentChange(eachPattern[12], patForRec[2]))
        sim14 = 100.00 -  abs(percentChange(eachPattern[13], patForRec[3]))
        sim15 = 100.00 -  abs(percentChange(eachPattern[14], patForRec[4]))
        sim16 = 100.00 -  abs(percentChange(eachPattern[15], patForRec[5]))
        sim17 = 100.00 -  abs(percentChange(eachPattern[16], patForRec[6]))
        sim18 = 100.00 -  abs(percentChange(eachPattern[17], patForRec[7]))
        sim19 = 100.00 -  abs(percentChange(eachPattern[18], patForRec[8]))
        sim20 = 100.00 -  abs(percentChange(eachPattern[19], patForRec[9]))

        sim21 = 100.00 -  abs(percentChange(eachPattern[20], patForRec[0]))
        sim22 = 100.00 -  abs(percentChange(eachPattern[21], patForRec[1]))
        sim23 = 100.00 -  abs(percentChange(eachPattern[22], patForRec[2]))
        sim24 = 100.00 -  abs(percentChange(eachPattern[23], patForRec[3]))
        sim25 = 100.00 -  abs(percentChange(eachPattern[24], patForRec[4]))
        sim26 = 100.00 -  abs(percentChange(eachPattern[25], patForRec[5]))
        sim27 = 100.00 -  abs(percentChange(eachPattern[26], patForRec[6]))
        sim28 = 100.00 -  abs(percentChange(eachPattern[27], patForRec[7]))
        sim29 = 100.00 -  abs(percentChange(eachPattern[28], patForRec[8]))
        sim30 = 100.00 -  abs(percentChange(eachPattern[29], patForRec[9]))
        ...
        if howSim > 70:
            ...
            #print("=====================================")
            #print(eachPattern)
            #print('Predicted outcome {}'.format(performanceAr[patdex]))
...
def graphRawFX():
    ...
    #adjusts the x axis labels to be slanted
...
print('Data lenght is',dataLength)
...
while toWhat < dataLength:
    ...
    print('Entire processing time took {} seconds'.format(totalTime))

    #moveOne = raw_input('Press enter to continue...')
    samps += 1
    toWhat += 1
    accuracyAverage = reduce(lambda x, y: x+y, accuracyArray) / len(accuracyArray)
    print('Backtested Accuracy is {} after {} samples'.format(accuracyAverage, samps))
